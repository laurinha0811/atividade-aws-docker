...
Passo a passo para instalar e configurar Docker em uma instância EC2 na AWS.

## Lançar uma instância EC2
Para criar uma instância EC2 no console da AWS é necessário seguir os seguintes passos: 
1. Ir para o AWS Management Console.
2. Navegar até o serviço EC2.
3. Clicar em Launch Instance.
4. Selecionar uma Amazon Machine Image (AMI). Para este tutorial, selecione Amazon Linux 2.
5. Escolha o tipo de instância. A t2.micro é suficiente para esse caso.
6. Em Configure Instance Details, verifique se a instância está dentro de uma VPC (rede privada).
...
- Acessar o console da AWS RDS e clicar em "Create Database".
...
- Defina um nome de usuário (por exemplo: laura) e uma senha segura.
- Certifique-se de configurar o RDS para estar na mesma VPC que a instância EC2.
- Clique em "Create Database" e aguarde até o banco de dados MySQL ser iniciado. Após a criação do banco de dados você verá uma tela semelhante a essa: 

![image](https://github.com/user-attachments/assets/f88d0556-4e99-467f-9302-769416068545)

## Conectar o WordPress ao RDS (MySQL)
Agora que temos o RDS MySQL pronto, o própximo passo será concectar a aplicação WordPress ao banco de dados durante o deploy. Para isso, vamos usar Docker Compose para a aplicação WordPress.
- Vamos criar um arquivo `docker-compose.yml`. Em resumo, esse arquivo ficará responsável por definir como o Docker deve iniciar os containers para o WordPress e contectar ao banco MySQL:
...
- `WORDPRESS_DB_USER`: é o nome de usuário do banco de dados (que configuramos no RDS).
- `WORDPRESS_DB_PASSWORD`: é a senha do banco de dados MySQL (também configurada no RDS).
- `WORDPRESS_DB_NAME`: é o nome do banco de dados MySQL..
- `ports`: é a configuração 80:80 faz o container WordPress rodar na porta 80, tornando o site acessível via HTTP.

Feito isso, podemos inicializar o conteiner WordPress. Vamos ao diretório onde foi criado o arquivo docker-compose.yml, e lá vamos executar o seguinte comando:
...
Feito a implantação, vamos conseguir acessar o Wordpress na porta 80 da nossa instância EC2. Para isso, vamos usar o URL associado à instância. Para confirmar que está funcionando:
...
- Selecionar a VPC onde a instância EC2 está localizada.
- Escolher as sub-redes nas zonas de disponibilidade, nesse caso as mesmas em que a EC2 esta rodando.
- Selecionar os grupos de segurança, lembre de garantir que ela tenha acesso de inbound NFS (porta 2049).
- Feito as configurações, basta clicar em Create.

Agora, vamos conectar à instância EC2. Para montar o EFS, instale os pacotes necessários:
...
- No painel esquerdo, vamos até a seção Load Balancers.
...
- Listeners: Protocolo: Selecione HTTP - Porta: porta padrão 80.
